...
export function Dashboard() {
  ...
  async function loadONVs() {
    ...
    const data = await contract.retrieveItemsListed()
  ...
  if (loadingState === 'loaded' && !onvs.length) return (<h1 className="py-10 px-20 text-3xl">No NFTs listed</h1>)
  return (
    <div>
      <div className="p-4">
        <h2 className="text-2xl py-2">Items List:</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
          {
            onvs.map((onv, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                <img src={onv.image} className="rounded" alt="ovn image" />
